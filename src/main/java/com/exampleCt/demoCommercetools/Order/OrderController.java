...
public class OrderController {
  ...
  /*  @PostMapping("/{id}")
   ...
   /* @GetMapping("/{orderId}/lineItems/{lineItemId}")
    public Address getLineItemShippingAddress(@PathVariable String orderId, @PathVariable String lineItemId) {
        String whereClause = "id=\"" + orderId + "\" or lineItems(id=\"" + lineItemId + "\")";
        return projectApiRoot.orders().get().addWhere(whereClause).executeBlocking().getBody().getResults()
                .stream()
                .filter(order -> order.getLineItems().stream().anyMatch(lineItem -> lineItemId.equals(lineItem.getId())))
                .findFirst()
